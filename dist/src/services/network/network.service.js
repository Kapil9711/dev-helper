...
class NetworkService {
    ...
    async discoverLocal() {
        console.log(chalk_1.default.yellow("🌐 Discovering Local Devices\n"));
        const subnet = (await (0, exec_1.exec)(`
...
`, true)).trim();
    ...
    async discoverSubnet(subnet) {
        console.log(chalk_1.default.yellow("🔍 Scanning subnet...\n"));
        await (0, exec_1.exec)(`
for ip in $(seq 1 254)
do
...
wait
`, true);
        const arp = await (0, exec_1.exec)(`arp -a`, true);
        const devices = arp.split("\n").filter(Boolean);
        console.log(chalk_1.default.yellow("📱 Devices"));
        for (const line of devices) {
            ...
            console.log(`${chalk_1.default.green(ip)} | ${mac || "Unknown MAC"}`);
        }
        console.log(chalk_1.default.green("\n✅ Discovery Complete\n"));
    }
    async inspectDevice(ip) {
        console.log(chalk_1.default.yellow("🖥 Device Inspection\n"));
        const hostname = (await (0, exec_1.exec)(`nslookup ${ip}`, true))
            .split("\n")
    ...
    async discoverHost(host) {
        console.log(chalk_1.default.yellow("🌐 Host Discovery\n"));
        const ip = (await (0, exec_1.exec)(`dig +short ${host} | head -n1`, true)).trim();
        const info = JSON.parse(await (0, exec_1.exec)(`curl -s ipinfo.io/${ip}`, true));
        ...
        console.log("\nPort Scan\n");
        await this.ports(host, {});
    }
    async ports(host, options) {
        try {
            console.log(chalk_1.default.cyan.bold("\n🔌 PORT SCAN\n"));
            console.log(chalk_1.default.yellow("🎯 Target"));
            ...
            console.log(chalk_1.default.yellow("🔎 Scanning"));
            ...
            for (let i = 0; i < ports.length; i += BATCH) {
                ...
                const results = await Promise.all(chunk.map(async ([port, service]) => {
                    try {
                        await (0, exec_1.exec)(`
nc \
-z \
...
`, true);
                ...
                process.stdout.write(`Progress : ${Math.min(i + BATCH, ports.length)}/${ports.length}\r`);
            }
            console.log("\n");
            console.log(chalk_1.default.yellow("📂 Open Ports"));
            if (!openPorts.length) {
                console.log(chalk_1.default.red("No open ports found"));
            }
            openPorts.forEach((port) => {
                console.log(`${chalk_1.default.green(port.port)} → ${port.service}`);
            });
            console.log("\n");
            ...
            if (openPorts.some((x) => x.port === 22)) {
                console.log(chalk_1.default.yellow("SSH reachable from internet"));
            }
            const databases = openPorts.filter((x) => [3306, 5432, 6379, 27017].includes(x.port));
            if (databases.length) {
                console.log(chalk_1.default.red("Database services exposed"));
            }
            console.log(chalk_1.default.green("\n✅ Scan complete\n"));
        }
        catch (error) {
            console.log(chalk_1.default.red("❌ Scan failed"));
            console.error(error);
        }
    }
    async trace(host) {
        try {
            ...
            console.log(chalk_1.default.yellow("🎯 Target"));
            console.log(`Host : ${host}\n`);
            const resolvedIp = (await (0, exec_1.exec)(`dig +short ${host} | head -n1`, true)).trim();
            console.log(chalk_1.default.yellow("🌍 DNS Resolution"));
            console.log(`Resolved IP : ${resolvedIp}\n`);
            console.log(chalk_1.default.gray("Fast tracing...\n"));
            const raw = await (0, exec_1.exec)(`
traceroute \
-m 8 \
-q 1 \
-w 200 \
...
`, true);
            ...
            console.log(chalk_1.default.gray(`Enriching ${uniqueIps.size} public hops...\n`));
            const ipCache = new Map();
            await Promise.all([...uniqueIps].map(async (ip) => {
                try {
                    const result = await (0, exec_1.exec)(`curl -s --max-time 1 ipinfo.io/${ip}`, true);
                    ipCache.set(ip, JSON.parse(result));
                }
                catch { }
            }));
            console.log(chalk_1.default.yellow("📍 Hop Details"));
            for (const hop of hops) {
                const cleaned = hop.trim();
                if (cleaned.includes("*")) {
                    timeoutCount++;
                    console.log(chalk_1.default.red(cleaned));
                    continue;
                }
                ...
                if (ipCache.has(ip)) {
                    ...
                    extra = chalk_1.default.gray(` | ${info.org || "Unknown"} | ${info.country || ""}`);
                }
                else if (ip) {
                    extra = chalk_1.default.gray(" | Private");
                }
            ...
            console.log(`Avg Latency    : ${latencyCount ? (latencyTotal / latencyCount).toFixed(2) : "N/A"} ms`);
            console.log("");
            console.log(chalk_1.default.yellow("🚨 Analysis"));
            if (timeoutCount > 2) {
                console.log(chalk_1.default.yellow("Packet filtering detected"));
            }
            console.log(chalk_1.default.green("\n✅ Trace complete\n"));
        }
        catch (error) {
            console.log(chalk_1.default.red("❌ Trace failed"));
            console.error(error);
        }
    }
    async ssl(domain) {
        try {
            ...
            const cert = await (0, exec_1.exec)(`
...
`, true);
            ...
            const serial = cert.match(/Serial Number:\s*([A-Fa-f0-9:\s]+)/)?.[1]?.trim() ||
                "Unknown";
            const signature = cert.match(/Signature Algorithm:\s*(.*)/)?.[1] || "Unknown";
            ...
            const protocol = await (0, exec_1.exec)(`
echo | openssl s_client \
-connect ${domain}:443 \
-servername ${domain} \
2>/dev/null |
grep "Protocol"
`, true);
            const cipher = await (0, exec_1.exec)(`
echo | openssl s_client \
-connect ${domain}:443 \
-servername ${domain} \
2>/dev/null |
grep "Cipher is"
`, true);
            const verify = await (0, exec_1.exec)(`
echo | openssl s_client \
-connect ${domain}:443 \
-servername ${domain} \
2>/dev/null |
...
`, true);
            const headers = await (0, exec_1.exec)(`curl -I -s https://${domain}`, true);
            const expiryDate = new Date(expiry);
            const issuedDate = new Date(issued);
            const daysLeft = Math.ceil((expiryDate.getTime() - Date.now()) / 86400000);
            const certAge = Math.floor((Date.now() - issuedDate.getTime()) / 86400000);
            ...
            console.log("");
            console.log(chalk_1.default.yellow("🛡 Trust"));
            console.log(`Self Signed   : ${selfSigned ? "Yes" : "No"}`);
            console.log(`Trust Status  : ${verify.trim()}`);
            console.log("");
            console.log(chalk_1.default.yellow("🌍 Domains"));
            ...
            console.log("");
            console.log(chalk_1.default.yellow("⚙ Transport"));
            console.log(`HSTS          : ${hsts ? "Enabled" : "Disabled"}`);
            console.log(`OCSP          : ${ocsp ? "Present" : "Not Found"}`);
            console.log("");
            console.log(chalk_1.default.yellow("🚨 Status"));
            if (daysLeft < 15) {
                console.log(chalk_1.default.red("Certificate expiring soon"));
            }
            else if (daysLeft < 45) {
                console.log(chalk_1.default.yellow("Monitor renewal"));
            }
            else {
                console.log(chalk_1.default.green("Healthy certificate"));
            }
            console.log(chalk_1.default.green("\n✅ SSL inspection complete\n"));
        }
        catch (error) {
            console.log(chalk_1.default.red("❌ SSL inspection failed"));
            console.error(error);
        }
    }
    async inspect(input) {
        try {
            ...
            const testUrl = `${input}${input.includes("?") ? "&" : "?"}nocache=${random}`;
            console.log(chalk_1.default.cyan.bold("\n🌐 NETWORK INSPECT"));
            console.log(chalk_1.default.gray(`Target : ${input}`));
    ...
    async showDns(domain) {
        console.log(chalk_1.default.yellow("📡 DNS"));
        const resolver = "8.8.8.8";
        console.log(chalk_1.default.gray("\nResolver Used:"));
        console.log(`  ${resolver} (Google DNS)`);
        const ns = await (0, exec_1.exec)(`dig NS +short ${domain}`, true);
        ...
        const cname = await (0, exec_1.exec)(`dig CNAME +short ${domain}`, true);
        console.log(chalk_1.default.gray("\nCNAME:"));
        console.log(cname.trim() || "  none");
        const records = await (0, exec_1.exec)(`dig A +short ${domain}`, true);
        console.log(chalk_1.default.gray("\nA Records:"));
        records
            .split("\n")
            .filter(Boolean)
            .forEach((ip) => {
            console.log(`  ${domain} → ${chalk_1.default.green(ip)}`);
        });
        console.log("");
    }
    async showIp(domain) {
        const ip = (await (0, exec_1.exec)(`dig +short ${domain} | head -n1`, true)).trim();
        console.log(chalk_1.default.yellow("🌍 IP INFO"));
        console.log(`Resolved IP : ${chalk_1.default.green(ip)}`);
        if (!ip) {
            console.log("");
            return;
        }
        const raw = await (0, exec_1.exec)(`curl -s ipinfo.io/${ip}`, true);
    ...
    async showRedirects(url) {
        console.log(chalk_1.default.yellow("↪ Redirect Chain"));
        const chain = await (0, exec_1.exec)(`
curl -sIL "${url}" |
...
`, true);
    ...
    async showHeaders(url) {
        console.log(chalk_1.default.yellow("🖥 Headers"));
        const headers = await (0, exec_1.exec)(`curl -I -s "${url}"`, true);
        headers
            .split("\n")
            .filter((line) => {
    ...
    async showStats(url, follow) {
        console.log(chalk_1.default.yellow(follow ? "📊 With Redirects" : "📊 Without Redirects"));
        const raw = await (0, exec_1.exec)(`
curl ${follow ? "-L" : ""} \
-o /dev/null \
-s \
...
"${url}"
`, true);
        const stats = Object.fromEntries(raw
            .trim()
            .split("\n")
            .map((line) => line.split(":")));
        ...
        if (total > 1) {
            console.log(chalk_1.default.red("⚠ Slow response detected"));
        }
        console.log("");
    }
}
